...
+ (NSString *)formatDateToken:(NSString *)token {
    ...
    if ([token isEqualToString:@"%a"]) { // Weekday as locale’s abbreviated name.
        ...
        result = @"EEE";
    } else if ([token isEqualToString:@"%w"]) { // Weekday as a decimal number, where 0 is Sunday and 6 is Saturday. 0, 1, ..., 6
        result = @"ccccc";
        ...
        result = @"ss";
    } else if ([token isEqualToString:@"%f"]) { // Microsecond as a decimal number, zero-padded on the left. 000000, 000001, ..., 999999
        result = @"SSSSSS";
    } else if ([token isEqualToString:@"%z"]) { // UTC offset in the form +HHMM or -HHMM (empty string if the the object is naive). (empty), +0000, -0400, +1030
        result = @"Z";
    } else if ([token isEqualToString:@"%Z"]) { // Time zone name (empty string if the object is naive). (empty), UTC, EST, CST
        result = @"z";
    } else if ([token isEqualToString:@"%j"]) { // Day of the year as a zero-padded decimal number. 001, 002, ..., 366
        result = @"DDD";
    } else if ([token isEqualToString:@"%U"]) { // Week number of the year (Sunday as the first day of the week) as a zero padded decimal number. All days in a new year preceding the first Sunday are considered to be in week 0. 00, 01, ..., 53 (6)
        result = @"ww";
    } else if ([token isEqualToString:@"%W"]) { // Week number of the year (Monday as the first day of the week) as a decimal number. All days in a new year preceding the first Monday are considered to be in week 0. 00, 01, ..., 53 (6)
        result = @"ww";
...
+ (NSString *) convertFormat:(NSString *)format {
    NSCharacterSet *alphabeticSet = [NSCharacterSet characterSetWithCharactersInString:@"abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ"];
